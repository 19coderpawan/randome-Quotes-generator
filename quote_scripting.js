...
let getquote=function(){
    let quotenum=Math.floor(Math.random()*(obj.length));
    let finalquote=obj[quotenum];
    return finalquote;
}
// let btn=document.getElementById("#btn_gen");


let printquote=function(){  
    let quote_container=document.getElementById("quotes_data");
    let dis= getquote();
    quote_container.innerHTML=`<p id="quote">${dis.quote}</p> <p id="source">${dis.source}</p>`;
...
document.getElementById("btn_gen").addEventListener("click",printquote);